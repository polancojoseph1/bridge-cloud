...
## 2026-04-14 - AutoFocus on Primary Inputs
**Learning:** For a chat application, the primary interaction is typing a message. When users navigate to an empty state or an existing chat, failing to auto-focus the text input requires an extra click/tap before they can start typing, creating unnecessary friction.
**Action:** Always add `autoFocus` to the primary text inputs in chat interfaces and empty states to enable instant keyboard interaction and reduce friction.
