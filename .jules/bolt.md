...
## 2024-05-22 - [Module-Level Dynamic Import Caching]
**Learning:** In 'hot' code paths (like `sendMessage` streaming pipelines), dynamic imports (`await import(...)`) can introduce minor overhead via repeated asynchronous microtasks and module registry lookups, even when cached by the runtime. Caching the module in a file-level variable prevents this overhead, but it can negatively impact developer experience by breaking Hot Module Replacement (HMR) during local development, as the cached variable holds a stale module reference.
**Action:** When implementing module-level caching for dynamic imports, document the trade-off regarding HMR interference, and ensure the performance gain is measurable enough in the target path to justify the potential DX hit.
