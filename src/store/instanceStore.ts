  ...
  persist(
    (set, get) => ({
      ...
      setInstanceConversation: (instanceId: string, conversationId: string) => {
        set(s => {
          // ⚡ Bolt: Replace map() with findIndex() to prevent unnecessary O(N) allocations for unmodified array items
      ...
      renameInstance: (instanceId: string, label: string) => {
        set(s => {
          // ⚡ Bolt: Replace map() with findIndex() to prevent unnecessary O(N) allocations
          const idx = s.instances.findIndex(i => i.instanceId === instanceId);
          if (idx === -1) return s;
          const instances = [...s.instances];
