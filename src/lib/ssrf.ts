...
export function isForbiddenHostname(hn: string): boolean {
  ...
  // IPv6 blocking
  if (cleanHn.includes(':')) {
    // 🛡️ Sentinel: Normalize IPv6 addresses to prevent zero-compression bypasses
    let normalized = cleanHn;
    try {
      normalized = new URL('http://[' + cleanHn + ']').hostname.slice(1, -1);
    } catch {
      // Ignore invalid IPv6 addresses and fall back to strict check
    }

    if (normalized === '::1') return true; // Loopback
    if (normalized === '::') return true; // Unspecified
    if (normalized.startsWith('::ffff:')) return true; // IPv4-mapped IPv6
    if (/^[fF][cCdDeEfF]/.test(normalized)) return true; // Unique local address (fc00::/7)
    if (/^[fF][eE][89aAbB]/.test(normalized)) return true; // Link-local (fe80::/10)
    if (normalized.startsWith('100:')) return true; // RFC 6666 discard
  }
