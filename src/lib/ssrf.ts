...
export function isForbiddenHostname(hn: string): boolean {
  let decodedHn = hn;
  try {
    // 🛡️ Sentinel: URL decode to prevent bypass via encoded characters (e.g. %2e for dot)
    decodedHn = decodeURIComponent(hn);
  } catch (e) {
    // ignore malformed URIs
  }

  // 🛡️ Sentinel: Strip trailing dots to prevent FQDN bypass (e.g. localhost.)
  const cleanHn = decodedHn.replace(/^\[|\]$/g, '').replace(/\.+$/, '').toLowerCase();
