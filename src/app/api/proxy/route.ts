...
export async function POST(req: NextRequest) {
  ...
  if (agentId === 'free') {
    ...
    // 🛡️ Sentinel: Prevent credential leak. Only use the cloud key if the cloud URL is explicitly configured.
    // Otherwise, fallback entirely to the user-provided URL and key.
    if (cloud && cloud.url) {
      targetUrl = cloud.url;
...
function createSSEToNDJSONTransform() {
  ...
  return new TransformStream({
    transform(chunk, controller) {
      ...
      // 🛡️ Sentinel: Enforce max buffer size to prevent Memory Exhaustion (DoS)
      if (buffer.length > MAX_BUFFER_SIZE) {
        controller.error(new Error('Stream buffer exceeded maximum size (DoS prevention)'));
        return;
      }
