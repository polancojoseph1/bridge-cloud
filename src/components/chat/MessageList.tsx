...
export default function MessageList({ conversationId }: MessageListProps) {
  ...
  const handleScroll = () => {
    ...
    // Check if this scroll event was programmatic
    if (expectedScrollTop.current !== null) {
      if (Math.abs(scrollTop - expectedScrollTop.current) <= 1) {
        // It's the programmatic scroll we initiated, clear it and ignore
        expectedScrollTop.current = null;
        return;
      }
      // If it doesn't match, it might be a user scroll that happened before our programmatic one finished.
      // We clear it and evaluate as a normal scroll.
      expectedScrollTop.current = null;
    }
  ...
  useEffect(() => {
    ...
    if (scrollRef.current && !isUserScrolledRef.current) {
      ...
      if (Math.abs(scrollRef.current.scrollTop - targetScrollTop) > 1) {
        // Use a ref for deterministic tracking of programmatic scrolls
