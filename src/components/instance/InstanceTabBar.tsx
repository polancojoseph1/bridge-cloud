...
const InstanceTab = memo(({ instance, isActive, canClose }: { instance: Instance; isActive: boolean; canClose: boolean }) => {
...
const MobileInstancePill = memo(({ instance, isActive, canClose }: { instance: Instance; isActive: boolean; canClose: boolean }) => {
  const setActive        = useInstanceStore(s => s.setActiveInstance);
  const close            = useInstanceStore(s => s.closeInstance);
  ...
  return (
    <>
      ...
      <div className="hidden lg:flex items-center h-9 flex-shrink-0 border-t border-[#1e3025] bg-[#0a1410] relative">
        ...
        >
          {instances.map(i => <InstanceTab key={`desktop-${i.instanceId}`} instance={i} isActive={activeInstanceId === i.instanceId} canClose={instancesLength > 1 && !i.isPinned} />)}
        </div>
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 flex items-center h-14 bg-[#0e1c14] border-t border-[#1e3025] safe-area-pb">
        <div
          ...
          style={{ scrollbarWidth: 'none' }}
        >
          {instances.map(i => <MobileInstancePill key={`mobile-${i.instanceId}`} instance={i} isActive={activeInstanceId === i.instanceId} canClose={instancesLength > 1} />)}
        </div>
