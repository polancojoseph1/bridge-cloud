...
// ⚡ Bolt Optimization:
// Wrapped child in React.memo and passed data as props.
// This prevents an O(N^2) rendering bottleneck where N children independently run O(N) .find() lookups.
// It also ensures that toggling the active tab only re-renders the changed tabs, not the entire list.
const InstanceTab = React.memo(function InstanceTab({
  instance,
  instancesLength,
  isActive,
  setActive,
...
// ⚡ Bolt Optimization: Memoized to maintain O(1) referential stability and prevent unnecessary re-renders when other tabs change state.
const MobileInstancePill = React.memo(function MobileInstancePill({
  instance,
  instancesLength,
  isActive,
  setActive,
