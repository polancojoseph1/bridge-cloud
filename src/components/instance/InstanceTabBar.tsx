...
const InstanceTab = memo(function InstanceTab({ instance, isActive, instancesLength, setActive, close }: { instance: Instance, isActive: boolean, instancesLength: number, setActive: (id: string) => void, close: (id: string) => void }) {
...
const MobileInstancePill = memo(function MobileInstancePill({ instance, isActive, instancesLength, setActive, close }: { instance: Instance, isActive: boolean, instancesLength: number, setActive: (id: string) => void, close: (id: string) => void }) {
  const dotColor = AGENT_DOT_COLORS[instance.agentId] ?? '#5c5c5c';
...
export default function InstanceTabBar() {
  ...
  const closeInstance     = useInstanceStore(s => s.closeInstance);
  ...
  return (
    <>
      ...
      <div className="hidden lg:flex items-center h-9 flex-shrink-0 border-t border-[#1e3025] bg-[#0a1410] relative">
        ...
        >
          {instances.map(i => <InstanceTab key={`desktop-${i.instanceId}`} instance={i} isActive={i.instanceId === activeInstanceId} instancesLength={instances.length} setActive={setActiveInstance} close={closeInstance} />)}
        </div>
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 flex items-center h-14 bg-[#0e1c14] border-t border-[#1e3025] safe-area-pb">
        <div
          ...
          style={{ scrollbarWidth: 'none' }}
        >
          {instances.map(i => <MobileInstancePill key={`mobile-${i.instanceId}`} instance={i} isActive={i.instanceId === activeInstanceId} instancesLength={instances.length} setActive={setActiveInstance} close={closeInstance} />)}
        </div>
