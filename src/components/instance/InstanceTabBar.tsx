...
/**
 * ⚡ Bolt Optimization: Added React.memo() and prop drilling
 * 💡 What: Replaced direct O(N) Zustand store lookups inside child components with direct props passed from parent, and memoized the component.
 * 🎯 Why: When global instances change, each child component previously performed an O(N) lookup.
 *         This caused O(N^2) time complexity and O(N) re-renders.
 * 📊 Impact: O(1) render cost for tabs, avoiding unnecessary global state recalculations in child components.
 */
const InstanceTab = memo(function InstanceTab({ instance, isActive, canClose, setActive, close }: InstanceTabProps) {
...
const MobileInstancePill = memo(function MobileInstancePill({ instance, isActive, canClose, setActive, close }: InstanceTabProps) {
  const dotColor = AGENT_DOT_COLORS[instance.agentId] ?? '#5c5c5c';
