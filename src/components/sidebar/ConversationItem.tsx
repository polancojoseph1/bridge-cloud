  ...
  return (
    ...
    >
      ...
      <button
        ...
        title={isConfirming ? "Confirm delete" : "Delete conversation"}
        className={cn(
          'flex-shrink-0 p-1 rounded opacity-0 group-hover:opacity-100',
          isConfirming ? 'text-[#e05c5c] opacity-100' : 'text-[#5c5c5c] hover:text-[#e05c5c]',
          'transition-all duration-150',
          'focus-visible:outline-none focus-visible:opacity-100 focus-visible:ring-1 focus-visible:ring-[#6c8cff]'
        )}
      >
        {isConfirming ? <Check className="w-3.5 h-3.5" /> : <Trash2 className="w-3.5 h-3.5" />}
      </button>
    </div>
  );
});
