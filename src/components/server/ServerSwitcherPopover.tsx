...
export default function ServerSwitcherPopover({ onClose }: Props) {
   ...
   * 💡 What: Replaced global store destructuring with individual, targeted selectors.
   * 🎯 Why: Destructuring `useServerStore()` subscribes the component to ALL store changes.
   *         This caused unnecessary re-renders when unrelated state (like connection status) changed.
   *         Using targeted selectors ensures we only re-render when needed.
   * 📊 Impact: Eliminates unnecessary re-renders of the popover component.
   */
