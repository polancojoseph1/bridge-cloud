...
export default function ServerSwitcherPopover({ onClose }: Props) {
  ...
  /**
   * ⚡ Bolt Optimization: Targeted Zustand Selectors
   * 💡 What: Replaced object destructuring (`const { a, b } = useStore()`) with individual targeted selectors.
   * 🎯 Why: Object destructuring subscribes the component to the entire store. This caused ServerSwitcherPopover
   *         to re-render unnecessarily on ANY store state change, even unrelated ones.
   * 📊 Impact: Prevents massive over-rendering. Component now only re-renders when profiles or active profile changes.
   */
